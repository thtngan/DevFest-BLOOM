$(document).ready(function() {
    ...
    if (user_role == 1) {
        $('#visiter').css({ 'display': 'none' })
        $('#funder').css({ 'display': "block" })
    }
});
var tomo = 0;
$('#donor tr').each(function() {
    var amount = Number($(this).find('td:nth-child(4)').text().replace(/\D/g, ''));
    tomo += amount;
})
$('#tomo span, #cur_money').html(tomo.toLocaleString());
var all_money = Number($('#price').text().replace(/\D/g, '')) * Number($('#portion').text());
$('#all_money').html(all_money.toLocaleString());
